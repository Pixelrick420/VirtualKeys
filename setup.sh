...
install_dependencies() {
    ...
    # Get Python version for specific venv package installation
    PYTHON_VERSION=$(get_python_version)
    print_status "Detected Python version: $PYTHON_VERSION"

    # Install the specific python3-venv package for the detected Python version
    VENV_PACKAGE="python${PYTHON_VERSION}-venv"
    print_status "Checking for venv package: $VENV_PACKAGE"

    if ! dpkg -l | grep -q "^ii  $VENV_PACKAGE "; then
        print_status "Installing $VENV_PACKAGE..."
        if sudo apt install -y "$VENV_PACKAGE" 2>/dev/null; then
            print_success "$VENV_PACKAGE installed successfully"
        else
            print_warning "Failed to install $VENV_PACKAGE, trying generic python3-venv..."
            install_package python3-venv
        fi
    else
        print_success "$VENV_PACKAGE is already installed"
    fi
...
setup_virtual_environment() {
    ...
    # First attempt: try creating venv with ensurepip
    print_status "Attempting to create virtual environment..."
    if python3 -m venv venv 2>&1 | tee /tmp/venv_setup.log; then
        print_success "Virtual environment created successfully"
    else
        # Check if the error is about ensurepip not being available
        if grep -q "ensurepip is not available" /tmp/venv_setup.log; then
            print_warning "ensurepip not available, creating venv without pip..."
            ...
            print_status "Installing pip manually in virtual environment..."
            curl -sS https://bootstrap.pypa.io/get-pip.py -o /tmp/get-pip.py
            if python /tmp/get-pip.py; then
                print_success "Pip installed manually in virtual environment"
            else
                print_error "Failed to install pip manually"
                return 1
            fi
